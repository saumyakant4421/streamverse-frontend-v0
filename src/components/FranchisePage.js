...
const FranchisePage = () => {
      ...
      try {
        ...
        setError(
          error.response?.data?.details ||
            "Failed to load franchise. Please try again."
        );
      } finally {
        setIsLoading(false);
      }
  ...
  if (!franchise)
    return <div className="error-message">Franchise not found</div>;

  const totalCast =
    Array.isArray(franchise.totalCast) &&
    franchise.totalCast.every(
      (p) => p && typeof p === "object" && p.name && p.id
    )
      ? franchise.totalCast
      : Array.from(
          new Map(
            (franchise.movies || [])
              ...
              .map((person, index) => {
                ...
                const id =
                  typeof person === "object" && person.id
                    ? person.id
                    : `cast-${index}-${name.replace(/\s+/g, "-")}`;
                const profile_path =
  ...
  const totalDirectors =
    Array.isArray(franchise.totalDirectors) &&
    franchise.totalDirectors.every(
      (p) => p && typeof p === "object" && p.name && p.id
    )
            ...
            (franchise.movies || [])
              ...
              .map((person, index) => {
                ...
                const id =
                  typeof person === "object" && person.id
                    ? person.id
                    : `director-${index}-${name.replace(/\s+/g, "-")}`;
                const profile_path =
                  typeof person === "object"
                    ? person.profile_path || null
                    : null;
    ...
    <div className="franchise-page">
      ...
      <div className="franchise-header">
        <div className="franchise-header-content">
          ...
          <p className="franchise-overview">
            {franchise.overview || "No overview available"}
          </p>
        </div>
        ...
        <div className="franchise-details">
          ...
          <p>
            {franchise.totalBoxOffice > 0
              ? `$${franchise.totalBoxOffice.toLocaleString()}`
              : "Not available"}
          </p>
        </div>
      </section>

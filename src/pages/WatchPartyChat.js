...
const WatchPartyChat = () => {
  ...
  const [userKeys, setUserKeys] = useState({ publicKey: null, privateKey: null });
  ...
  const generateKeyPair = async () => {
    ...
    return { publicKey: sodium.to_base64(publicKey), privateKey: sodium.to_base64(privateKey) };
  };
...
const fetchWatchParty = async () => {
    try {
      ...
      const usersResponse = await watchPartyService.get(`/${watchPartyId}/users`);
      const userPublicKeys = usersResponse.data.map(user => user.publicKey || user.publicKey); // Handle potential null
      ...
      if (!localStorage.getItem('publicKey') || !localStorage.getItem('privateKey')) {
        ...
        setUserKeys(keys);
        console.log(`Sending public key for user ${user.uid} to ${watchPartyId}`);
        const postResponse = await watchPartyService.post(`/${watchPartyId}/users`, { userId: user.uid, publicKey: keys.publicKey });
        ...
        // Re-send public key to ensure it's in Firestore
        console.log(`Re-sending public key for user ${user.uid} to ${watchPartyId}`);
        await watchPartyService.post(`/${watchPartyId}/users`, { userId: user.uid, publicKey: localStorage.getItem('publicKey') });
      }
    } catch (err) {
      console.error('Error fetching watch party:', err.message, err.response?.data);
...
const sendMessage = async (e) => {
    ...
    try {
      ...
      const encryptedMessages = await Promise.all(allPublicKeys.map(async (publicKey) => {
      ...
      const response = await watchPartyService.post(`/${watchPartyId}/message`, payload);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(messagesQuery, async (snapshot) => {
      ...
      // Sort by the timestamp of the first message in the 'messages' array
      docs = docs.sort((a, b) => {
        const getTime = (msg) => {
          const t = msg && msg.messages && msg.messages[0] && msg.messages[0].timestamp;
          if (!t) return 0;
          if (typeof t.toDate === 'function') return t.toDate().getTime(); // Firestore Timestamp
          return new Date(t).getTime(); // JS Date or ISO string
        };
        return getTime(a) - getTime(b);
      });
      // Decrypt all messages asynchronously
      const decryptedMessages = await Promise.all(docs.map(async msg => ({
        ...msg,
  ...
  useEffect(() => {
    // Get all unique userIds from messages
  ...
  // Encrypt a message for a recipient using libsodium
  async function encryptMessage(message, recipientPublicKeyBase64) {
    ...
    const encryptedMessage = sodium.crypto_secretbox_easy(messageUint8, nonce, symmetricKey);
  ...
  async function decryptMessage(msg) {
    try {
      ...
      // Find the message encrypted for this user
      const m = msg.messages.find(
        m => m.recipientPublicKey === myPublicKey
      );
      ...
      // Decrypt the symmetric key using our keypair
      const symmetricKey = sodium.crypto_box_seal_open(encryptedSymmetricKey, publicKey, privateKey);
      if (!symmetricKey) return '[Decryption failed]';
      // Decrypt the message
      const decrypted = sodium.crypto_secretbox_open_easy(encryptedMessage, nonce, symmetricKey);
  ...
  if (!user) {
    return (
      <div className="watchparty-chat-page">
        ...
        <Link to="/login" className="back-button">Go to Login</Link>
      </div>
    );
  }
      ...
      <div className="chat-container">
        ...
        <div className="chat-messages">
          <div className="chat-messages-inner">
            <div style={{ textAlign: 'center', color: '#888', margin: '8px 0', fontSize: '0.95em' }}>
            ...
            {messages.map((message) => (
              ...
              >
                <span className="chat-message-user">{userMap[message.userId] || message.userId}:</span>

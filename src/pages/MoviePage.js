...
import { movieService, userService } from "../lib/services"; // Import services
...
const MovieDetailPage = () => {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      try {
        ...
        const movieResponse = await movieService.get(`/${id}`);
        setMovie(movieResponse.data);

        const similarResponse = await movieService.get(`/${id}/similar`);
        setSimilarMovies(similarResponse.data || []);

        const streamingResponse = await movieService.get(
          `/${id}/streaming?region=US`
        );
        setStreamingData(streamingResponse.data || null);
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      const response = await movieService.post(`/comments/${id}`, {
        comment: newComment,
      });
  ...
  const checkWatchlistAndWatchedStatus = async () => {
    ...
    try {
      const watchlistResponse = await userService.get(
        `/watchlist?uid=${user.uid}`
      );
      setIsInWatchlist(
        watchlistResponse.data.some((item) => item.id === Number(id))
      );

      const watchedResponse = await userService.get(`/watched?uid=${user.uid}`);
      setIsWatched(watchedResponse.data.some((item) => item.id === Number(id)));
    } catch (error) {
      console.error(
        "Error checking watchlist/watched status:",
    ...
    try {
      if (isInWatchlist) {
        await userService.delete(
          `/watchlist/remove?uid=${user.uid}&movieId=${movie.id}`
        );
  ...
  const handleAddToWatched = async () => {
    ...
    try {
      if (isWatched) {
        await userService.delete(
          `/watched/remove?uid=${user.uid}&movieId=${movie.id}`
        );
        setIsWatched(false);
      ...
      console.error(
        "Error updating watched list:",
        error.response?.data || error.message
      );
      toast.error(
        error.response?.data?.error || "Failed to update watched list"
      );
    } finally {
  ...
  const fetchStreamingData = async () => {
    try {
      ...
      const response = await movieService.get(`/${id}/streaming?region=US`);
                  ...
                  {comments.length === 0 ? (
                    ...
                    comments.map((comment) => {
                      ...
                      try {
                        ...
                        console.warn(
                          "Invalid date for comment:",
                          comment.id,
                          err
                        );
                      }
                      const avatarInitial = comment.username
              ...
              <div className="streaming-providers">
                ...
                streamingData.flatrate.length === 0 ? (
                  <div className="no-providers-message">
                    <p>
                      Currently, there are no streaming providers for the movie.
                    </p>
                  </div>
                ) : (
                  <>
                      ...
                      {Object.entries(ottProviderMap).map(([key, service]) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              isAvailable &&
                              handleStreamingServiceClick(serviceUrl)
                            }
                          >

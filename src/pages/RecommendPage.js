...
import { FiRefreshCw, FiSend, FiRotateCcw, FiThumbsUp, FiThumbsDown } from 'react-icons/fi';
...
  const [chatBotResponseText, setChatBotResponseText] = useState("Hello! I'm MovieAI. Ask me for movie recommendations!");
...
    if (userId) {
      console.log(`Setting up Firestore listener for chat history for user: ${userId}`);
...
            const data = doc.data();
            // console.log('Firestore message:', JSON.stringify(data)); // Removed verbose log
            messages.push({ id: doc.id, ...data });
          });
          if (isMounted) {
            setChatHistory(messages.sort((a, b) => (a.timestamp?.toMillis() || 0) - (b.timestamp?.toMillis() || 0)));
            if (messages.length > 0) {
              const latestBotResponse = messages[messages.length - 1].text_response;
              const latestBotRecommendations = messages[messages.length - 1].bot || [];
              setChatBotResponseText(latestBotResponse || "Here are your movie recommendations!");
              setChatRecommendations(latestBotRecommendations);
            }
          }
        },
        (err) => {
          console.error('Firestore error fetching chat history:', err.code, err.message);
          if (isMounted) setError(`Failed to fetch chat history: ${err.message}`);
        }
      );
      return () => {
...
      const response = await recommendationService.post('/personalized', { userId }, { timeout: 120000 });
...
    } catch (error) {
      console.error('Error fetching personalized recommendations:', error.message, error.response?.status, error.response?.data);
      setError(`Failed to fetch recommendations: ${error.message}. Please try again.`);
    } finally {
      setLoading(false);
    }
  };
...
      setChatHistory(prev => [...prev, { id: Date.now(), user: chatQuery, timestamp: { toMillis: () => Date.now() } }]);
      setChatQuery('');
...
      const textResponse = response.data.text_response || 'Here are your movie recommendations!';
...
      setRetryCount(0);
    } catch (error) {
      console.error('Error fetching chat recommendations:', error.message, error.response?.status, error.response?.data);
      if (error.code === 'ECONNABORTED' && retryCount < MAX_RETRIES) {
        console.log(`Retrying chat recommendations (attempt ${retryCount + 1}/${MAX_RETRIES})`);
        setRetryCount(retryCount + 1);
        setTimeout(() => fetchChatRecommendations(), 1000 * (retryCount + 1));
      } else {
        const errorMessage = error.response?.data?.error || `Failed to fetch chat recommendations: ${error.message}`;
...
                <span>Fetching your recommendations... (may take up to 30 seconds)</span>
              </div>
            )}
...
                      <h3>{movie.title}</h3>
                      <span className="year">{movie.release_date?.substring(0, 4) || movie.year || 'N/A'}</span>
                    </div>
                  </div>
                ))}
              </div>
...
                  <p className="placeholder">No chat history yet. Start by entering a query!</p>
                )}
...
                        <p className="text-xs">{formatTimestamp(chat.timestamp)}</p>
                      </div>
                    </div>
                    {chat.bot && (
...
                          <p>{chat.text_response || 'Here are your movie recommendations!'}</p>
                          <p className="text-xs">{formatTimestamp(chat.timestamp)}</p>
                        </div>
                      </div>
                    )}
...
                  disabled={loading}
                  onKeyPress={(e) => e.key === 'Enter' && fetchChatRecommendations()}
                />
                <button className="send-button" onClick={fetchChatRecommendations} disabled={loading}>
                  <FiSend /> Send
                </button>
              </div>
...
                    <button
                      key={`suggestion-${query}-${index}`} // More robust key
...
                      className="movie-card"
                      onClick={() => handleMovieClick(movie.tmdb_id || movie.id)}
                    >
                      <div className="movie-poster">
                        {movie.poster_path ? (
                          <img
...
                        <p className="text-sm description">{movie.description || 'No description available.'}</p>
                        <p className="text-blue-600 justification">
                          <strong>Why:</strong> {movie.justification || 'Matches your query.'}
                        </p>
                        <div className="feedback-buttons">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleFeedback(movie.tmdb_id || movie.id, 'thumbs_up');
                            }}
                          >
                            <FiThumbsUp />
                          </button>
...
                            onClick={(e) => {
                              e.stopPropagation();
                              handleFeedback(movie.tmdb_id || movie.id, 'thumbs_down');
                            }}
                          >
                            <FiThumbsDown />
                          </button>
